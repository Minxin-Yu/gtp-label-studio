...
CENTRAL_SERVICE_URL = get_env(
    'CENTRAL_SERVICE_URL', 'http://tp-central-service:6150')
...
class CentralApi(BaseHTTPAPI):
    ...
    def config(self, project, use_ground_truth=False):
        print(project.id)
        url = '/service/service_info?task_size=100&project=' + str(project.id)
        print(url)
        print('...............')

        return self._request(url, method='GET', verbose=False, timeout=TIMEOUT_PREDICT)
