...
export const ConfigData = ({ config, taskCount }) => {
  if (config.tasks !== undefined) {
    if(config.tasks[taskCount] !== undefined){
      ...
      let TaskIdDisplay = <text>{config.tasks[taskCount].subTask}</text>;
      let CompareSubTasks = '';
      ...
      return (
        <div>
          <br />
          <Card style={{ marginTop: 0 }} header={config.tasks[0].name + '-' + (taskCount + 1)}
          >
            <DescriptionList className={cn('ml').elem('summary')}>
              ...
              <DescriptionList.Item term="Sub Task" >
                {/* <a href={SubTaskLink}>{config.tasks[taskCount].subTask}</a> */}
